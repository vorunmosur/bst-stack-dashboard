...
st.set_page_config(page_title="Ohmium Fleet Monitor", layout="wide")
...
st.title("⚡ Ohmium Fleet Performance Dashboard")
 
# 2. Universal Data Processing Engine
...
def load_and_process(file, mode):
    ...
    # Health Engine Logic (40, 30, 20, 10)
...
with st.sidebar:
    ...
    st.header("1. Fleet Data Ingestion")

    sys_mode = st.radio("System Type", ["BST", "MTS"])

    uploaded_files = st.file_uploader(f"Upload Multiple {sys_mode} Files", type=["csv"], accept_multiple_files=True)
 
if not uploaded_files:

    st.info("👈 Please upload stack files to begin.")
...
tab1, tab2, tab3 = st.tabs(["🏛️ Fleet View & Cell Analysis", "🔍 Deep-Dive Diagnostics", "📈 Comparative Trends (Dual Axis)"])
 
# --- TAB 1: FLEET VIEW & CELL ANALYSIS ---

with tab1:

    st.subheader("Fleet Health Overview (Distance to 100)")
    ...
    for i, (name, df) in enumerate(all_data.items()):
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            mode = "gauge+number", value = avg_h, domain = {'x': [0, 1], 'y': [0, 1]},
        ...
        with grid_cols[i % 3]:
            ...
            st.markdown(f'<div class="grid-card-text">Gap to Ideal: {gap:.1f} pts</div>', unsafe_allow_html=True)
 
    st.divider()

    st.subheader("Individual Cell Voltage Analysis")
    ...
    sel_time_c = st.select_slider("Select Inspection Timestamp (Cell)", options=time_strs_c, key="cell_slider")
    ...
    fig_cells.add_hline(y=IDEAL_REF, line_dash="dash", line_color="red", annotation_text="Ideal Reference (1.7V)")
...
with tab2:
    ...
    target_d = st.selectbox("Select Stack for Diagnostics", list(all_data.keys()), key="diag_target")
    ...
    st.table(pd.DataFrame(rank_list).sort_values("Avg Health", ascending=False))
    ...
    fig_ht.add_trace(go.Scatter(x=df_d['Time'], y=df_d['health_score'], name="Health Score", line=dict(color='black')))

    fig_ht.update_layout(height=400, yaxis_range=[0,105], hovermode="x unified")

    st.plotly_chart(fig_ht, use_container_width=True)
 
    # 3. Sustained Red Alert Log

    st.subheader("🚨 Sustained Red Alert Log (10+ Points)")
    ...
    if not sustained.empty: st.warning(f"Sustained alerts detected:"); st.table(sustained.reset_index(drop=True))
    ...
    st.download_button("Download Diagnostic Report (CSV)", data=csv, file_name=f"diag_{target_d}_{sel_time_d}.csv", mime='text/csv')
...
with tab3:
    ...
    all_p_options = ["None"] + sorted(list(set().union(*(d.columns for d in all_data.values()))))

    c1, c2 = st.columns(2)

    p1 = c1.selectbox("Left Axis Parameter", [p for p in all_p_options if p != "None"], index=0)

    p2 = c2.selectbox("Right Axis Parameter", all_p_options, index=0)
    ...
    if comp_list:
        ...
        if p2 != "None": fig_dual.update_yaxes(title_text=f"<b>{p2}</b> (Dotted)", secondary_y=True)
