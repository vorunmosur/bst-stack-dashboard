...
# Replace the icon URL with your local Ohmium logo path if preferred
...
st.markdown("""
    ...
    /* Title styling to accommodate logo */
    .title-container { display: flex; align-items: center; gap: 20px; }
    .title-container img { height: 50px; }
    </style>
    """, unsafe_allow_html=True)

# Branding: Sidebar Logo and Main Title Logo
st.logo(LOGO_URL, icon_image=LOGO_URL)

st.markdown(f"""
    <div class="title-container">
        <img src="{LOGO_URL}">
        <h1>Ohmium Stack Performance Dashboard</h1>
    </div>
    """, unsafe_allow_html=True)
...
with tab2:
    ...
    st.divider()
    target_d = st.selectbox("Select Stack for Alert Log & Root Cause", list(all_data.keys()), key="diag_target")
    ...
    # Status Panel & Root Cause Inspection
...
with tab3:
    ...
    c1, c2 = st.columns(2)
    # Defaulting primary axis to avg_cell_voltage
