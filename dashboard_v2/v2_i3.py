...
st.title("⚡ Ohmium Stack Performance Dashboard")

# 2. Data Processing Engine (BST & MTS Support)
@st.cache_data
def load_and_process(file, mode):
    ...
    # Health Engine (Weights: 40, 30, 20, 10)
    base = {
...
with tab1:
    ...
    fig_dev.update_layout(title=f"Cell Voltage Deviation from 1.7V at {sel_time_c}", yaxis_title="Deviation (V)", height=350)
    st.plotly_chart(fig_dev, use_container_width=True)

# --- TAB 2: DEEP-DIVE DIAGNOSTICS (FIXED FOR MULTI-STACK) ---
with tab2:
    # 1. Ranking Table
    ...
    # 2. Health Score Plot (Multi-Select Fix)
    ...
    fig_ht.update_layout(height=400, yaxis_range=[0,105], hovermode="x unified", title="Health Score Trends Over Time")
    st.plotly_chart(fig_ht, use_container_width=True)

    # 3. Sustained Alert Log & Root Cause (Single Select for Clarity)
    st.divider()
    target_d = st.selectbox("Select Stack for Detailed Alert Log & Root Cause", list(all_data.keys()), key="diag_target")
...
# --- TAB 3: COMPARATIVE TRENDS (Dashed Relationship Tool) ---
with tab3:
